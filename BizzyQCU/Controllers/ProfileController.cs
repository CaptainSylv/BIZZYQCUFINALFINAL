            ...
            try
            {
                ...
                if (wantsPasswordChange)
                {
                    if (string.IsNullOrWhiteSpace(request.CurrentPassword) ||
                        ...
                        string.IsNullOrWhiteSpace(request.ConfirmPassword))
                    {
                        return Json(new { success = false, message = "Current, new, and confirm password are required." });
                    }
                ...
                if (wantsPasswordChange)
                {
                    var passwordUpdated = db.UpdateUserPassword(userId, request.CurrentPassword, request.NewPassword);
                    if (!passwordUpdated)
                    {
                        return Json(new { success = false, message = "Current password is incorrect or password update failed." });
                    }
                }
